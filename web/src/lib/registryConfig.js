...
export async function fetchRegistryConfig(connection) {
    ...
    if (!info)
        throw new Error("RegistryConfig PDA not found on chain");
    const data = info.data;
    let off = 8 + 1; // skip discriminator + bump
    const admin = new PublicKey(data.subarray(off, off + 32));
    off += 32;
    const treasury = new PublicKey(data.subarray(off, off + 32));
    off += 32;
    const regFeeSol = data.readBigUInt64LE(off);
    off += 8;
    const regFeeSpl = data.readBigUInt64LE(off);
    off += 8;
    const feeSplMint = new PublicKey(data.subarray(off, off + 32));
    off += 32;
    const paymentBps = data.readUInt16LE(off);
    off += 2;
    const regLifetime = data.readBigInt64LE(off);
    off += 8;
