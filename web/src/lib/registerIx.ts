...
function ixDiscriminator(method: string): Uint8Array {
  return sha256(TEXT(`global:${method}`)).slice(0, 8);
}

export function phoneRecordPda(phoneE164: string): PublicKey {
  return PublicKey.findProgramAddressSync(
    [TEXT("phone"), TEXT(phoneE164)],
    PHONE_REGISTRY_PROGRAM,
  )[0];
}

export function reverseIndexPda(owner: PublicKey): PublicKey {
  return PublicKey.findProgramAddressSync(
    [TEXT("by_owner"), owner.toBuffer()],
    PHONE_REGISTRY_PROGRAM,
  )[0];
}
...
interface BuildArgs {
  ...
  owner: PublicKey;
  treasury: PublicKey;
  attestationHash: Uint8Array; // 32 bytes
  attestationKind: number;     // 1 = Android Keystore, 3 = web-delegated
  acceptedMints?: PublicKey[];
  preferredReceive?: PublicKey;
  flags?: number;
}
 ...
 */
export function buildRegisterWithSolFeeIx(args: BuildArgs): TransactionInstruction {
